  ...
  const savejournalDefinition = function(datatosave, callback){
      let parent;
        ...
        db.transaction(function(tx) {
            ...
            tx.executeSql('INSERT INTO journalDefinitionDetails(parentguid, journalDefinationId, accountCode, amount, canChangeAccountCode, canChangeAmount, canChangeIsCredited, compulsory, isCredited) VALUES (?,?,?,?,?,?,?,?,?)',[item.parentguid, parent, item.accountCode, parseInt(item.amount), item.canChangeAccountCode, item.canChangeAmount, item.canChangeIsCredited, item.compulsory, item.isCredited], function (tx, results) {
               console.log({results});
                callback('success')              
            }, function (e, r) {
               console.log({e});
