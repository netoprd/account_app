...
const getchatofaccountbyId = function (id) {
  ...
  const savejournalDefinition = (datatosave) =>{
        ...
        db.transaction(function(tx) {
            ...
            tx.executeSql('INSERT INTO journalDefinationDeitail(journalDefinationId, accountCode, amount, canChangeAccountCode, canChangeAmount, canChangeIsCredited, compulsory, isCredited) VALUES' + '('+5 +',' +''+ item.accountCode+',' +''+  item.amount+',' +''+ item.canChangeAccountCode+',' +''+   item.canChangeAmount +',' +''+  item.canChangeIsCredited +',' +''+ item.compulsory +',' +''+  item.isCredited +');')
        }
           // let allchild = [];
