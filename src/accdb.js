...
function initTables() {
   db.transaction(function (tx) {
      ...
      tx.executeSql('CREATE TABLE IF NOT EXISTS journal(id INTEGER PRIMARY KEY, approvedBy BOOLEAN DEFAULT FALSE,  approvedOn VARCHAR, autoGenerated BOOLEAN DEFAULT FALSE, journalDescription VARCHAR, journalguid VARCHAR, narration VARCHAR, transactionDate DATE, transactionRef VARCHAR)');

      tx.executeSql('CREATE TABLE IF NOT EXISTS journalDetails(id INTEGER PRIMARY KEY, journalid NUMBER,  accountCode VARCHAR, credit NUMBER, debit NUMBER, journalDefinitionId NUMBER, parentguid VARCHAR, sn VARCHAR, transactionNarration VARCHAR)');
...
const createjournal = function(datatosave, callback){
   ...
   let parent;
   db.transaction(function (tx) {
      tx.executeSql('INSERT INTO journal(approvedBy,  approvedOn, autoGenerated, journalDescription, journalguid, narration, transactionDate, transactionRef) VALUES (?,?,?,?,?,?,?,?)', [datatosave.approvedBy, datatosave.approvedOn, datatosave.autoGenerated, datatosave.journalDescription, datatosave.journalguid, datatosave.narration, datatosave.transactionDate, datatosave.transactionRef], function (tx, results) {
         if (results.insertId) {
            parent = results.insertId
            for (const item of datatosave.journalDetails) {
               tx.executeSql('INSERT INTO journalDetails(journalid, accountCode, credit, debit, journalDefinitionId, parentguid, sn, transactionNarration) VALUES (?,?,?,?,?,?,?)', [parent, item.accountCode, item.credit, item.debit, item.journalDefinitionId, item.parentguid, item.sn, item.transactionNarration], function (tx, results) {
                  // console.log({ results });
                  // callback('success')
               }, function (e, r) {
                  console.log({ e });
