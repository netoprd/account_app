...
export default function EditJournal() {
    ...
    useEffect(() => {
        ...
        function callback(r) {
            if (r.message === 'success') {
                ...
                function callback(r) {
                    if (r.length !== 0) {
                        setJournalDefinition(r)
                        setJournalDefinitionDetails(r?.journalDefinitionDetails)
                        if (r.canAddMoreItems === "true") {
                            setcanAdd(true)
                        }
                        if (r.canRemoveMoreItems === "false") {
    ...
    useEffect(() => {
        if (journal) {
            journal.transactionRef && setValue("transactionRef", journal.transactionRef);
            journal.transactionDate && setValue("transactionDate", moment(journal?.transactionDate).format("YYYY-MM-DD"));
            journal.journalDescription && setValue("journalDescription", journal.journalDescription);
            journal.narration && setValue("narration", (journal.narration));
            journal.autoGenerated && setValue("autoGenerated", journal.autoGenerated === "false" ? false : true);
        }
    }, [journal?.journalsDetails ? null : journal])
    ...
    const addToJournalDetails = () => {
        ...
        reset2()
        if (journalDefinition.canAddMoreItems === "false") {
            setcanAdd(false)
        }
    ...
    const editJournal = (journal) => {
        ...
        const jourDef = journalDefinitionDetails.find(x => x.id === journal.journalDefinitionId);
        if (jourDef) {
            if (jourDef.canChangeAmount === "false") {
                setchangeAMountCode(true)
            }
            if (jourDef.canChangeAmount === "false" || jourDef.canChangeAccountCode === "false") {
    ...
    const submitJournal = () => {
        const payload = {
            guid: journal.guid,
            journalDefinitionGuid: journal.journalDefinitionGuid,
        ...
        console.log({ payload })
        localStorage.setItem("journalEdit", JSON.stringify(payload));
        dbData.edithjournal(payload.guid, payload, callback)
        function callback(r) {
            if (r === 'success') {
                history("/listjournal")
    ...
    return (
        <>
            <div className="content-wrapper mt-5">
                ...
                {canAdd && <div className="card mt-2">
                    <div className="card-body">
